...
void svm_learn_classification(DOC **docs, double *class, long int totdoc,
                ...
                optimization if not NULL. Array must be of size totdoc. */
{
  ...
  for (i = 0; i < totdoc; i++) { /* various inits */
    ...
    if (class[i] > 0) {
      learn_parm->svm_cost[i] =
          learn_parm->svm_c * learn_parm->svm_costratio * docs[i]->costfactor;
      label[i] = 1;
      trainpos++;
    } else if (class[i] < 0) {
  ...
  if (alpha) {
    ...
    if (kernel_cache && (kernel_parm->kernel_type != LINEAR)) {
      ...
      for (i = 0; i < totdoc;
           i++) /* fill rest of kernel cache with bounded SV */
        if ((alpha[i] == learn_parm->svm_cost[i]) &&
            (kernel_cache_space_available(kernel_cache)))
          cache_kernel_row(kernel_cache, docs, i, kernel_parm);
    }
    ...
    update_linear_component(docs, label, index2dnum, alpha, a, index2dnum,
                            totdoc, totwords, kernel_parm, kernel_cache, lin,
                            aicache, weights);
  ...
  if (transduction) {
    ...
    if (verbosity >= 1)
      printf("\nDeactivating Shrinking due to an incompatibility with the "
  ...
  if (transduction && learn_parm->compute_loo) {
    ...
    if (verbosity >= 1)
      printf("\nCannot compute leave-one-out estimates for transductive "
             "learner.\n\n");
  }

  if (learn_parm->remove_inconsistent && learn_parm->compute_loo) {
    learn_parm->compute_loo = 0;
    printf("\nCannot compute leave-one-out estimates when removing "
           "inconsistent examples.\n\n");
  }

  if (learn_parm->compute_loo && ((trainpos == 1) || (trainneg == 1))) {
    learn_parm->compute_loo = 0;
    printf("\nCannot compute leave-one-out with only one example in one "
           "class.\n\n");
  }
  ...
  /* train the svm */
  iterations = optimize_to_convergence(
      docs, label, totdoc, totwords, learn_parm, kernel_parm, kernel_cache,
      &shrink_state, model, inconsistent, unlabeled, a, lin, c, &timing_profile,
  ...
  if (verbosity >= 1) {
    ...
    if (verbosity >= 2) {
      printf(
          "Runtime in cpu-seconds: %.2f (%.2f%% for kernel/%.2f%% for "
          "optimizer/%.2f%% for final/%.2f%% for update/%.2f%% for "
          "model/%.2f%% for check/%.2f%% for select)\n",
  ...
  if (learn_parm->compute_loo) {
    ...
    for (heldout = 0; (heldout < totdoc); heldout++) {
      if (learn_parm->rho * a_fullset[heldout] * r_delta_sq +
        ...
        optimize_to_convergence(docs, label, totdoc, totwords, learn_parm,
                                ...
                                &timing_profile, &maxdiff, heldout, (long)2);
    ...
    model->loo_recall =
        (1.0 - (double)loo_count_pos / (double)trainpos) * 100.0;
...
/* model:       Returns learning result (assumed empty before called) */
{
  ...
  for (i = 0; i < totdoc; i++) {
    j = 2 * totdoc - 1 - i;
    docs[i] =
        create_example(i, 0, 0, docs_org[i]->costfactor, docs_org[i]->fvec);
    label[i] = +1;
    c[i] = value[i];
    docs[j] =
        create_example(j, 0, 0, docs_org[i]->costfactor, docs_org[i]->fvec);
    label[j] = -1;
    c[j] = value[i];
  }
  totdoc *= 2;
  ...
  if (*kernel_cache) {
    kernel_cache_size =
        (*kernel_cache)->buffsize * sizeof(CFLOAT) / (1024 * 1024);
  ...
  if ((kernel_parm->kernel_type == LINEAR) && (*kernel_cache)) {
    printf("WARNING: Using a kernel cache for linear case will slow "
           "optimization down!\n");
  }
                ...
                optimization if not NULL. Array must be of size totdoc. */
{
  ...
  if (learn_parm->sharedslack)   /* if shared slacks are used, they must */
    for (i = 0; i < totdoc; i++) /*  be used on every constraint */
      if (!docs[i]->slackid) {
        perror("Error: Missing shared slacks definitions in some of the "
               "examples.");
        exit(0);
      }
  ...
  */
  ...
  if (learn_parm->remove_inconsistent) {
    ...
    printf("'remove inconsistent' not available in this mode. Switching option "
           "off!");
    fflush(stdout);
  }
  ...
  if (learn_parm->sharedslack)
    iterations = optimize_to_convergence_sharedslack(
        docs, label, totdoc, totwords, learn_parm, kernel_parm, kernel_cache,
        &shrink_state, model, a, lin, c, &timing_profile, &maxdiff);
  else
    iterations = optimize_to_convergence(
        docs, label, totdoc, totwords, learn_parm, kernel_parm, kernel_cache,
        &shrink_state, model, inconsistent, unlabeled, a, lin, c,
  ...
  if (learn_parm->sharedslack) {
    ...
    compute_shared_slacks(docs, label, a, lin, c, index2dnum, learn_parm, slack,
                          alphaslack);
    loss = 0;
    model->at_upper_bound = 0;
    svsetnum = 0;
  ...
  if ((verbosity >= 1) && (!learn_parm->skip_final_opt_check)) {
    if (learn_parm->sharedslack) {
      ...
      printf("Number of non-zero slack variables: %ld (%ld slacks have "
             "non-zero alpha)\n",
...
/* retrain: selects training mode (1=regular / 2=holdout) */
{
  ...
  qp.opt_xinit =
      (double *)my_malloc(sizeof(double) * learn_parm->svm_maxqpsize);
  ...
  for (; retrain && (!terminate); iteration++) {
    ...
    for (jj = 0; (j = working2dnum[jj]) >= 0; jj++) { /* clear working set */
      if ((chosen[j] >=
           (learn_parm->svm_maxqpsize /
            minl(learn_parm->svm_maxqpsize, learn_parm->svm_newvarsinqp))) ||
    ...
    if (retrain == 2) {
      choosenum = 0;
      for (jj = 0; (j = working2dnum[jj]) >= 0;
           jj++) { /* fully clear working set */
        chosen[j] = 0;
      }
      clear_index(working2dnum);
      for (i = 0; i < totdoc;
           i++) { /* set inconsistent examples to zero (-i 1) */
      ...
      if (learn_parm->biased_hyperplane) {
        eq = 0;
        for (i = 0; i < totdoc;
             i++) { /* make sure we fulfill equality constraint */
          eq += a[i] * label[i];
        }
      ...
      if (iteration % 101) {
        ...
        if ((minl(learn_parm->svm_newvarsinqp,
          ...
          already_chosen = select_next_qp_subproblem_grad(
              ...
              (long)(minl(learn_parm->svm_maxqpsize - choosenum,
                     ...
                     2),
              learn_parm, inconsistent, active2dnum, working2dnum, selcrit,
              selexam, kernel_cache, 1, key, chosen);
        ...
        choosenum += select_next_qp_subproblem_grad(
            ...
            learn_parm, inconsistent, active2dnum, working2dnum, selcrit,
            selexam, kernel_cache, 0, key, chosen);
        ...
        choosenum += select_next_qp_subproblem_rand(
            ...
            minl(learn_parm->svm_maxqpsize - choosenum,
                 learn_parm->svm_newvarsinqp),
            learn_parm, inconsistent, active2dnum, working2dnum, selcrit,
            selexam, kernel_cache, key, chosen, iteration);
      }
    }
    ...
    if (kernel_cache)
      cache_multiple_kernel_rows(kernel_cache, docs, working2dnum, choosenum,
                                 kernel_parm);
    ...
    if (retrain != 2) {
      optimize_svm(docs, label, unlabeled, inconsistent, 0.0, chosen,
                   active2dnum, model, totdoc, working2dnum, choosenum, a, lin,
                   c, learn_parm, aicache, kernel_parm, &qp, &epsilon_crit_org);
    }

    if (verbosity >= 2)
      t3 = get_runtime();
    update_linear_component(docs, label, active2dnum, a, a_old, working2dnum,
                            totdoc, totwords, kernel_parm, kernel_cache, lin,
                            aicache, weights);
    ...
    if (verbosity >= 2)
      t4 = get_runtime();
    supvecnum =
        calculate_svm_model(docs, label, unlabeled, lin, a, a_old, c,
    ...
    if (iteration > (bestmaxdiffiter + learn_parm->maxiter)) {
      ...
      retrain = 0;
      if (verbosity >= 1)
        printf("\nWARNING: Relaxing KT-Conditions due to slow progress! "
               "Terminating!\n");
    }
    ...
    if ((!retrain) && (inactivenum > 0) &&
      ...
      t1 = get_runtime();
      reactivate_inactive_examples(
          label, unlabeled, a, shrink_state, lin, c, totdoc, totwords,
          iteration, learn_parm, inconsistent, docs, kernel_parm, kernel_cache,
          model, aicache, weights, maxdiff);
    ...
    if ((!retrain) && (transduction)) {
      ...
      if (verbosity == 1)
        printf("done\n");
      retrain = incorporate_unlabeled_examples(
          model, label, inconsistent, unlabeled, a, lin, totdoc, selcrit,
      ...
      activenum = shrink_problem(
          docs, learn_parm, shrink_state, kernel_parm, active2dnum,
          last_suboptimal_at, iteration, totdoc,
          maxl((long)(activenum / 10), maxl((long)(totdoc / 500), 100)), a,
          inconsistent);
      inactivenum = totdoc - activenum;
      if ((kernel_cache) && (supvecnum > kernel_cache->max_elems) &&
          ((kernel_cache->activenum - activenum) >
           maxl((long)(activenum / 10), 500))) {
    ...
    if ((!retrain) && learn_parm->remove_inconsistent) {
      ...
      if (learn_parm->remove_inconsistent == 1) {
        ...
        retrain = identify_misclassified(lin, label, unlabeled, totdoc, model,
                                         &inconsistentnum, inconsistent);
      } else if (learn_parm->remove_inconsistent == 3) {
        retrain =
            identify_one_misclassified(lin, label, unlabeled, totdoc, model,
                                       &inconsistentnum, inconsistent);
      }
      if (retrain) {
...
long optimize_to_convergence_sharedslack(
    DOC **docs, long int *label, long int totdoc, long int totwords,
    LEARN_PARM *learn_parm, KERNEL_PARM *kernel_parm,
    KERNEL_CACHE *kernel_cache, SHRINK_STATE *shrink_state, MODEL *model,
    double *a, double *lin, double *c, TIMING *timing_profile, double *maxdiff)
/* docs: Training vectors (x-part) */
/* label: Training labels/value (y-part, zero if test example for
                         transduction) */
/* totdoc: Number of examples in docs/label */
...
/* maxdiff: returns maximum violation of KT-conditions */
{
  ...
  for (; retrain && (!terminate); iteration++) {
    ...
    if (iteration % 101) {
      slackset =
          select_next_qp_slackset(docs, label, a, lin, slack, alphaslack, c,
                                  learn_parm, active2dnum, &maxsharedviol);
      if ((!(iteration % 100)) || (!slackset) ||
        ...
        i = 0;
        for (jj = 0; (j = working2dnum[jj]) >= 0;
             jj++) { /* clear old part of working set */
        ...
        if ((minl(learn_parm->svm_newvarsinqp,
          ...
          already_chosen = select_next_qp_subproblem_grad(
              ...
              (long)(minl(learn_parm->svm_maxqpsize - choosenum,
                     ...
                     2),
              learn_parm, inconsistent, active2dnum, working2dnum, selcrit,
              selexam, kernel_cache, (long)1, key, chosen);
          choosenum += already_chosen;
        }
        choosenum += select_next_qp_subproblem_grad(
            label, unlabeled, a, lin, c, totdoc,
            minl(learn_parm->svm_maxqpsize - choosenum,
                 learn_parm->svm_newvarsinqp - already_chosen),
            learn_parm, inconsistent, active2dnum, working2dnum, selcrit,
            selexam, kernel_cache, (long)0, key, chosen);
        ...
        choosenum = select_next_qp_subproblem_grad(
            label, unlabeled, a, lin, c, totdoc, learn_parm->svm_maxqpsize,
            learn_parm, ignore, active2dnum, working2dnum, selcrit, selexam,
      ...
      choosenum += select_next_qp_subproblem_rand(
          ...
          minl(learn_parm->svm_maxqpsize - choosenum,
               learn_parm->svm_newvarsinqp),
          learn_parm, inconsistent, active2dnum, working2dnum, selcrit, selexam,
          kernel_cache, key, chosen, iteration);
    }
    ...
    for (jj = 0; (i = working2dnum[jj]) >= 0;
         jj++) /* recompute sums of alphas */
    ...
    for (jj = 0; (i = active2dnum[jj]) >= 0; jj++)
      learn_parm->svm_cost[i] =
          a[i] + (learn_parm->svm_c - alphaslack[docs[i]->slackid]);
    model->at_upper_bound = 0;
    ...
    retrain = check_optimality_sharedslack(
        docs, model, label, a, lin, c, slack, alphaslack, totdoc, learn_parm,
        maxdiff, epsilon_crit_org, &misclassified, active2dnum,
        last_suboptimal_at, iteration, kernel_parm);
...
/* Do optimization on the working set. */
{
  ...
  compute_matrices_for_optimization(
      docs, label, unlabeled, exclude_from_eq_const, eq_target, chosen,
      active2dnum, working2dnum, model, a, lin, c, varnum, totdoc, learn_parm,
  ...
  for (i = 0; i < varnum; i++) {
    ...
    if(a_v[i]>=(learn_parm->svm_cost[working2dnum[i]]-learn_parm->epsilon_a)) {
...
void compute_matrices_for_optimization(
    DOC **docs, long int *label, long int *unlabeled,
    long *exclude_from_eq_const, double eq_target, long int *chosen,
    long int *active2dnum, long int *key, MODEL *model, double *a, double *lin,
    double *c, long int varnum, long int totdoc, LEARN_PARM *learn_parm,
    CFLOAT *aicache, KERNEL_PARM *kernel_parm, QP *qp) {
  ...
  if (verbosity >= 3) {
    fprintf(stdout,
            "Computing qp-matrices (type %ld kernel [degree %ld, rbf_gamma %f, "
            "coef_lin %f, coef_const %f])...",
  ...
  qp->opt_n = varnum;
  qp->opt_ce0[0] =
      -eq_target; /* compute the constant for equality constraint */
  ...
  for (i = 0; i < varnum; i++) {
    ...
    for (j = i + 1; j < varnum; j++) {
      ...
      /* compute quadratic part of objective function */
      qp->opt_g[varnum * i + j] =
          (double)label[ki] * (double)label[kj] * kernel_temp;
      qp->opt_g[varnum * j + i] =
          (double)label[ki] * (double)label[kj] * kernel_temp;
    }
...
/* of alpha. */
{
  ...
  return (model->sv_num -
          1); /* have to substract one, since element 0 is empty*/
}
...
/* Check KT-conditions */
{
  ...
  if (kernel_parm->kernel_type == LINEAR) { /* be optimistic */
    ...
    learn_parm->epsilon_shrink =
        learn_parm->epsilon_shrink * 0.7 + (*maxdiff) * 0.3;
  }
  retrain = 0;
  (*maxdiff) = 0;
  (*misclassified) = 0;
  ...
  for (ii = 0; (i = active2dnum[ii]) >= 0; ii++) {
    ...
    if ((a[i] > learn_parm->epsilon_a) && (dist > target)) {
      if ((dist - target) > (*maxdiff)) { /* largest violation */
        (*maxdiff) = dist - target;
        if (verbosity >= 5)
          printf("sid %ld: dist=%.2f, target=%.2f, slack=%.2f, a=%f, "
                 "alphaslack=%f\n",
                 docs[i]->slackid, dist, target, slack[docs[i]->slackid], a[i],
    ...
    if ((a[i] <= learn_parm->epsilon_a) &&
      ...
      last_suboptimal_at[i] = iteration; /* not likely optimal */
               ...
               (fabs(dist_noslack - target) > -learn_parm->epsilon_shrink)) {
      last_suboptimal_at[i] = iteration; /* not at lower bound */
    } else if ((alphaslack[docs[i]->slackid] >= ex_c) &&
               (a[i] > learn_parm->epsilon_a) &&
               (fabs(target - dist) > -learn_parm->epsilon_shrink)) {
      last_suboptimal_at[i] = iteration; /* not likely optimal */
    }
  }
  /* termination criterion */
...
/* compute the value of shared slacks and the joint alphas */
{
  ...
  for (jj = 0; (i = active2dnum[jj]) >= 0;
       jj++) { /* recompute slack variables */
...
long identify_misclassified(double *lin, long int *label, long int *unlabeled,
  ...
  for (i = 0; i < totdoc; i++) {
    dist =
        (lin[i] - model->b) * (double)label[i]; /* 'distance' from hyperplane*/
...
long identify_one_misclassified(double *lin, long int *label,
  ...
  for (i = 0; i < totdoc; i++) {
    if ((!inconsistent[i]) && (!unlabeled[i])) {
      dist = (lin[i] - model->b) *
             (double)label[i]; /* 'distance' from hyperplane*/
      if (dist < maxdist) {
        maxdist = dist;
        maxex = i;
      }
...
long incorporate_unlabeled_examples(MODEL *model, long int *label,
  ...
  if (learn_parm->transduction_posratio >= 0) {
    ...
    posratio =
        (double)orgpos / (double)(orgpos + orgneg); /* use ratio of pos/neg */
  }                                                 /* in training data */
  ...
  for (i = 0; i < totdoc; i++) {
    ...
    if ((!unlabeled[i]) &&
        (a[i] > (learn_parm->svm_cost[i] - learn_parm->epsilon_a))) {
      /*      printf("Ubounded %ld (class %ld, unlabeled
    ...
    for (i = 0; i < totdoc; i++) { /* set upper bounds on vars */
      if (unlabeled[i]) {
        if (label[i] == 1) {
          ...
          learn_parm->svm_cost[i] =
              learn_parm->svm_c * learn_parm->svm_unlabbound;
        }
      }
    }
    if (verbosity >= 1) {
  ...
  if ((transductcycle % check_every) == 0) {
    ...
    if ((!unsupaddnum1) && (!unsupaddnum2)) {
      ...
      printf("%ld positive -> Switching labels of %ld POS / %ld NEG unlabeled "
             "examples.",
             upos, unsupaddnum1, unsupaddnum2);
      fflush(stdout);
    }
...
long select_next_qp_subproblem_grad(
    long int *label, long int *unlabeled, double *a, double *lin, double *c,
    long int totdoc, long int qp_size, LEARN_PARM *learn_parm,
    long int *inconsistent, long int *active2dnum, long int *working2dnum,
    double *selcrit, long int *select, KERNEL_CACHE *kernel_cache,
    long int cache_only, long int *key, long int *chosen)
 ...
 those for which the kernel evaluations are cached. */
{
  ...
  for (i = 0; (j = active2dnum[i]) >= 0; i++) {
    ...
    if (valid && (!((a[j] <= (0 + learn_parm->epsilon_a)) && (s < 0))) &&
        (!((a[j] >= (learn_parm->svm_cost[j] - learn_parm->epsilon_a)) &&
           (s > 0))) &&
        (!chosen[j]) && (label[j]) && (!inconsistent[j])) {
      selcrit[activedoc] =
          (double)label[j] * (learn_parm->eps - (double)label[j] * c[j] +
  ...
  for (k = 0;
       (choosenum < (qp_size / 2)) && (k < (qp_size / 2)) && (k < activedoc);
       k++) {
...
long select_next_qp_subproblem_rand(
    long int *label, long int *unlabeled, double *a, double *lin, double *c,
    long int totdoc, long int qp_size, LEARN_PARM *learn_parm,
    long int *inconsistent, long int *active2dnum, long int *working2dnum,
    double *selcrit, long int *select, KERNEL_CACHE *kernel_cache,
    long int *key, long int *chosen, long int iteration)
/* Use the feasible direction approach to select the next
   ...
   problem. */
{
  ...
  for (i = 0; (j = active2dnum[i]) >= 0; i++) {
    ...
    if ((!((a[j] <= (0 + learn_parm->epsilon_a)) && (s < 0))) &&
        (!((a[j] >= (learn_parm->svm_cost[j] - learn_parm->epsilon_a)) &&
           (s > 0))) &&
        (!inconsistent[j]) && (label[j]) && (!chosen[j])) {
...
/* returns the slackset with the largest internal violation */
{
  ...
  for (ii = 0; (i = active2dnum[ii]) >= 0; ii++) {
    ...
    if (alphaslack[docs[i]->slackid] >= ex_c) {
      dist =
          (lin[i]) * (double)label[i] + slack[docs[i]->slackid]; /* distance */
      target =
          -(learn_parm->eps - (double)label[i] * c[i]); /* rhs of constraint */
      if ((a[i] > learn_parm->epsilon_a) && (dist > target)) {
        if ((dist - target) > maxdiff) { /* largest violation */
          maxdiff = dist - target;
...
void select_top_n(double *selcrit, long int range, long int *select,
  ...
  for (i = 0; (i < n) && (i < range);
       i++) { /* Initialize with the first n elements */
    for (j = i; j >= 0; j--) {
   ...
   minshrink variables can be removed. */
{
  ...
  if ((change >= minshrink) /* shrink only if sufficiently many candidates */
      && (shrink_state->deactnum <
          shrink_state->maxhistory)) { /* and enough memory */
...
void reactivate_inactive_examples(
    long int *label, long int *unlabeled, double *a, SHRINK_STATE *shrink_state,
    double *lin, double *c, long int totdoc, long int totwords,
    long int iteration, LEARN_PARM *learn_parm, long int *inconsistent,
    DOC **docs, KERNEL_PARM *kernel_parm, KERNEL_CACHE *kernel_cache,
    MODEL *model, CFLOAT *aicache, double *weights, double *maxdiff)
            ...
            values for linear kernel! */
{
  ...
  if (kernel_parm->kernel_type == LINEAR) { /* special linear case */
    ...
    for (t = shrink_state->deactnum - 1; (t >= 0) && shrink_state->a_history[t];
         t--) {
      if (verbosity >= 2) {
        printf("%ld..", t);
        fflush(stdout);
    ...
    if (!inconsistent[i]) {
      ...
      if ((a[i] > (0 + learn_parm->epsilon_a)) && (a[i] < ex_c)) {
        ...
        shrink_state->active[i] = 1;
                     ...
                     ->sharedslack) { /* make all active when sharedslack */
        shrink_state->active[i] = 1;
      }
    }
  }
  if (kernel_parm->kernel_type != LINEAR) { /* update history for non-linear */
    ...
    for (t = shrink_state->deactnum - 2; (t >= 0) && shrink_state->a_history[t];
         t--) {
...
/* Takes the values from the cache if available. */
{
  ...
  if (kernel_cache &&
      (kernel_cache->index[docnum] != -1)) { /* row is cached? */
    ...
    for (i = 0; (j = active2dnum[i]) >= 0; i++) {
      if (kernel_cache->totdoc2active[j] >= 0) { /* column is cached? */
        buffer[j] =
            kernel_cache->buffer[start + kernel_cache->totdoc2active[j]];
      } else {
    ...
    if (cache) {
      ...
      for (j = 0; j < kernel_cache->activenum; j++) { /* fill cache */
        ...
        if ((kernel_cache->index[k] != -1) && (l != -1) && (k != m)) {
          cache[j] =
              kernel_cache
                  ->buffer[kernel_cache->activenum * kernel_cache->index[k] +
                           l];
        } else {
   ...
   examples marked 0 in after. */
{
  ...
  kernel_cache->max_elems =
      (long)(kernel_cache->buffsize / kernel_cache->activenum);
   ...
   Efficiently, IMCL, 2000. */
{
      ...
      if ((learn_parm->rho * a[i] * r_delta_sq + xi) >= 1.0) {
        if (learn_parm->xa_depth > 0) { /* makes assumptions */
          sim = distribute_alpha_t_greedily(
              sv2dnum, svnum, docs, a, i, label, kernel_parm, learn_parm,
              (double)((1.0 - xi - a[i] * r_delta_sq) / (2.0 * a[i])));
        }
        if ((learn_parm->xa_depth == 0) ||
            ((a[i] * kernel(kernel_parm, docs[i], docs[i]) + a[i] * 2.0 * sim +
              xi) >= 1.0)) {
          looerror++;
          if (label[i] > 0) {
            looposerror++;
  ...
  (*precision) =
      (((double)totposex - (double)looposerror) /
   ...
   computing a better bound using a greedy optimzation strategy. */
{
  ...
  for (d = 0; d < learn_parm->xa_depth; d++) {
    ...
    if (!allskip) {
      for (k = 0; k < svnum; k++) {
        cache[d * svnum + k] =
            kernel(kernel_parm, docs[sv2dnum[best_ex[d]]], docs[sv2dnum[k]]);
      }
    }
...
void write_alphas(char *alphafile, double *a, long int *label,
                  long int totdoc) {
  FILE *alphafl;
  long i;
