...
/* only semi-definite. */
{
  ...
  if (!primal) { /* allocate memory at first call */
    ...
    buffer = (double *)my_malloc(sizeof(double) *
                                 ((nx + 1) * 2 * (nx + 1) * 2 + nx * nx +
  ...
  result = optimize_hildreth_despo(
      ...
      /* (long)PRIMAL_OPTIMAL, */
      (long)0, (long)0, lindep_sensitivity, qp->opt_g, qp->opt_g0, qp->opt_ce,
      qp->opt_ce0, qp->opt_low, qp->opt_up, primal, qp->opt_xinit, dual,
  ...
  if ((result != PRIMAL_OPTIMAL) || (!(roundnumber % 31)) || (progress <= 0)) {
    ...
    result = optimize_hildreth_despo(
        qp->opt_n, qp->opt_m, opt_precision, (*epsilon_crit),
        learn_parm->epsilon_a, (long)maxiter, (long)PRIMAL_OPTIMAL,
        (long)SMALLROUND, lindep_sensitivity, qp->opt_g, qp->opt_g0, qp->opt_ce,
        qp->opt_ce0, qp->opt_low, qp->opt_up, primal, qp->opt_xinit, dual,
        nonoptimal, buffer, &progress);
    if (verbosity >= 3) {
...
int optimize_hildreth_despo(n, m, precision, epsilon_crit, epsilon_a, maxiter,
                            goal, smallround, lindep_sensitivity, g, g0, ce,
                            ce0, low, up, primal, init, dual, lin_dependent,
                            buffer, progress)
                     ...
                     before and after */
{
  ...
  dual_old =
      &(buffer[n + (n + m) * 2 * (n + m) * 2 + (n + m) * 2 + n + m + n * n]);
      ...
      if (ce[b1] == ce[b2]) {
        if (g0_b1 <= g0_b2) { /* set b1 to upper bound */
          ...
          start[b2] = init[b2] + t;
                  ...
                  (g[b2 * n + b2] > 0))) { /* (ce[b1] != ce[b2]) */
        /* printf("case +!\n"); */
        t = ((ce[b2] / ce[b1]) * g0[b1] - g0[b2] +
             ce0[0] *
                 (g[b1 * n + b1] * ce[b2] / ce[b1] - g[b1 * n + b2] / ce[b1])) /
                ((ce[b2] * ce[b2] / (ce[b1] * ce[b1])) * g[b1 * n + b1] +
                 g[b2 * n + b2] - 2 * (g[b1 * n + b2] * ce[b2] / ce[b1])) -
            init[b2];
        changed = 1;
        if ((up[b2] - init[b2]) < t) {
          t = up[b2] - init[b2];
  ...
  if ((m > 0) && ((fabs(g[b1 * n + b1]) < lindep_sensitivity) ||
  ...
  /* in case both examples are linear dependent */
           ...
           (fabs(g[b1 * n + b1] / g[b1 * n + b2] -
                 g[b1 * n + b2] / g[b2 * n + b2]) < lindep_sensitivity)) {
    /* printf("Case lindep\n"); */
    add += 0.078274;
  }
  ...
  if ((!changed) || (n_indep > 1)) {
    result = solve_dual(n_indep, m, precision, epsilon_crit, maxiter, g_new,
                        g0_new, ce_new, ce0_new, low_new, up_new, primal, d, d0,
...
int solve_dual(n, m, precision, epsilon_crit, maxiter, g, g0, ce, ce0, low, up,
               primal, d, d0, ig, dual, dual_old, temp, goal)
  ...
  */
  ...
  while ((retrain) && (maxviol > 0) && (iter < (scalemaxiter * maxfaktor))) {
    ...
    for (i = 0; (i < n); i++) { /* check primal KT-Conditions */
      ...
      /*    printf("HIDEOtemp: a[%ld]=%.30f, dist=%.6f, b=%f,
...
void linvert_matrix(matrix, depth, inverse, lindep_sensitivity,
                    lin_dependent) double *matrix;
long depth;
...
void lprint_matrix(matrix, depth) double *matrix;
long depth;
{
  long i, j;
  for (i = 0; i < depth; i++) {
...
void ladd_matrix(matrix, depth, scalar) double *matrix;
long depth;
double scalar;
{
  long i, j;
...
void lcopy_matrix(matrix, depth, matrix2) double *matrix;
long depth;
double *matrix2;
{
  long i;
...
void lswitch_rows_matrix(matrix, depth, r1, r2) double *matrix;
long depth, r1, r2;
{
  long i;
  double temp;
...
void lswitchrk_matrix(matrix, depth, rk1, rk2) double *matrix;
long depth, rk1, rk2;
{
  long i;
  double temp;
