...
/* calculate the kernel function between two vectors */
{
  ...
  case POLY: /* polynomial */
    return (
        pow(kernel_parm->coef_lin * sprod_ss(a, b) + kernel_parm->coef_const,
  ...
  case SIGMOID: /* sigmoid neural net */
    return (
        tanh(kernel_parm->coef_lin * sprod_ss(a, b) + kernel_parm->coef_const));
...
SVECTOR *create_svector(WORD *words, char *userdefined, double factor) {
  ...
  if (userdefined) {
    vec->userdefined =
        (char *)my_malloc(sizeof(char) * (strlen(userdefined) + 1));
...
SVECTOR *create_svector_n(double *nonsparsevec, long maxfeatnum,
                          char *userdefined, double factor) {
  return (create_svector_n_r(nonsparsevec, maxfeatnum, userdefined, factor, 0));
}

SVECTOR *create_svector_n_r(double *nonsparsevec, long maxfeatnum,
                            char *userdefined, double factor,
                            double min_non_zero) {
  SVECTOR *vec;
  long fnum, i;
...
SVECTOR *add_list_ss(SVECTOR *a) { return (add_list_ss_r(a, 0)); }
   ...
   than min_non_zero to zero. */
{
  ...
  if (a) {
    ...
    concat_write = concat;
    for (i = 0; (i < length - 1) && (concat_write->wnum != concat_read->wnum);
         i++) {
      concat_write++;
      concat_read++;
    }
...
/* scale sparse vector a by factor */
{
  ...
  if (a->userdefined) {
    userdefined =
        (char *)my_malloc(sizeof(char) * (strlen(a->userdefined) + 1));
   ...
   not initialized. */
{
  ...
  for (i = 0; i < MIN(n, matrix->n); i++) {
    matrix->element[i] =
        (double *)realloc(matrix->element[i], sizeof(double) * m);
  }
  matrix->n = n;
  matrix->m = m;
  return (matrix);
...
/* Given a positive-definite symmetric matrix A[0..n-1][0..n-1], this routine
   constructs its Cholesky decomposition, A = L · LT . On input, only the upper 
   riangle of A need be given; A is not modified. The Cholesky factor L is
   eturned in the lower triangle. */
{
  int i, j, k, n;
  double sum;
  MATRIX *L;
...
/* Given a positive-semidefinite symmetric matrix A[0..n-1][0..n-1], this
   routine finds a subset of rows and colums that is linear independent. To do
   this, it constructs the Cholesky decomposition, A = L · LT. On input, only 
   he upper triangle of A need be given; A is not modified. The routine returns
    vector in which non-zero elements indicate the linear independent subset.
   psilon is the amount by which the diagonal entry of L has to be greater than
   ero. */
{
  int i, j, k, n;
  double sum, *indep;
...
/* For column vector v and matrix A (assumed to match in size), computes
   w^T=v^T*A */
{
  int i, j;
  double sum;
...
MODEL *copy_model(MODEL *model) {
  ...
  for (i = 1; i < model->sv_num; i++) {
    ...
    newmodel->supvec[i] = create_example(
        ...
        model->supvec[i]->costfactor, copy_svector(model->supvec[i]->fvec));
  }
  if (model->lin_weights) {
    newmodel->lin_weights =
...
int parse_document(char *line, WORD *words, double *label, long *queryid,
  ...
  while ((!space_or_null((int)line[pos])) && line[pos])
    pos++;
  while ((pos += read_word(line + pos, featurepair)) && (featurepair[0]) &&
         (wpos < max_words_doc)) {
    /* printf("%s\n",featurepair); */
...
int check_learning_parms(LEARN_PARM *learn_parm, KERNEL_PARM *kernel_parm) {
  if ((learn_parm->skip_final_opt_check) &&
      (kernel_parm->kernel_type == LINEAR)) {
    printf("\nIt does not make sense to skip the final optimality check for "
           "linear kernels.\n\n");
    learn_parm->skip_final_opt_check = 0;
  }
  if ((learn_parm->skip_final_opt_check) && (learn_parm->remove_inconsistent)) {
    printf("\nIt is necessary to do the final optimality check when removing "
           "inconsistent \nexamples.\n");
    return (0);
  }
  ...
  if ((learn_parm->svm_maxqpsize < learn_parm->svm_newvarsinqp)) {
    printf("\nMaximum size of QP-subproblems [%ld] must be larger than the "
           "number of\n",
           learn_parm->svm_maxqpsize);
  ...
  if (learn_parm->rho < 0) {
    printf("\nThe parameter rho for xi/alpha-estimates and leave-one-out "
           "pruning must\n");
    printf("be greater than zero (typically 1.0 or 2.0, see T. Joachims, "
           "Estimating the\n");
    printf(
  ...
  if ((learn_parm->xa_depth < 0) || (learn_parm->xa_depth > 100)) {
    printf("\nThe parameter depth for ext. xi/alpha-estimates must be in "
           "[0..100] (zero\n");
    printf("for switching to the conventional xa/estimates described in T. "
           "Joachims,\n");
    printf("Estimating the Generalization Performance of an SVM Efficiently, "
           "ICML, 2000.)\n");
  }
  return (1);
}
